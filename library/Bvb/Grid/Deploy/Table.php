...
class Bvb_Grid_Deploy_Table extends Bvb_Grid implements Bvb_Grid_Deploy_DeployInterface
{
     ...
     * @var bool
     */
    protected $_useKeyEventsOnFilters = true;
    /**
     * Extra Rows
     *
     * @var array
    ...
    /**
     * To add search button in filter.
     * 
     * @var string  
     */
    protected $_searchButton = '&nbsp;';
    ...
    protected function _buildHeader()
    {
        ...
        if (count($this->_filters) > 0) {
            ...
            if (($this->getInfo('noFilters') != 1 || $this->_allowAdd == 1)
                ...
                $translateObject = Zend_Registry::get('Zend_Translate');
                if (strlen($final1) > 5 || $this->getUseKeyEventsOnFilters() == false) {
                    if ($this->getUseKeyEventsOnFilters() === false && $this->getInfo('noFilters') != 1) {
                        $final1 .= "<button onclick=\"_" . $this->getGridId() . "gridChangeFilters(1)\">"
                                 . $this->__($translateObject->translate('search')) . "</button>";
                    }
                    ...
                    $this->_searchButton = str_replace("{{value}}", $final1, $this->_temp['table']->extra());
    ...
    protected function _buildFiltersTable($filters)
    {
        ...
        foreach ($filters as $filter) {
            ...
            //Check extra fields
            if ($flagSearchButtonSet == 0 && $filter['type'] == 'extraField' && $filter['position'] == 'left') {
                //Replace values
                $filterValue = isset($filter['value']) ? $filter['value'] : '';
                if($index == 1) {
                    $grid .= str_replace(
                        array('{{value}}', "{{colspan}}"),
                        array($filterValue . $this->_searchButton, $colspan),
                        $this->_temp['table']->filtersLoop()
                    );
                    $flagSearchButtonSet = 1;
                }
            }
                    ...
                    if ($noOrder == 1) {
                        ...
                        if ($this->getInfo("ajax") !== false) {
                            ...
                            if ($this->getAlwaysShowOrderArrows() === true && $this->getShowOrderImages() === true) {
                                ...
                                if($gridId=='createlaneGrid' || $gridId=='maintenancelaneGrid')
                                {
	                                $title['value'].= '<br/>';
    	                            $grid .= str_replace(
                                    array('{{value}}', "{{colspan}}"),
    ...
    protected function _buildGridTable($grids)
    {
        ...
        foreach ($grids as $value) {
            ...
            foreach ($value as $final) {
			    ...
			    $final['value']= html_entity_decode($final['value']);
    ...
    protected function _pagination()
    {
        ...
        if (count($this->_paginationInterval) > 0 && $this->getTotalRecords() > 0) {
            ...
            if ($this->getInfo('ajax') !== false) {
                // Reset the start of pagination when changing the perpage limit
                $urlData = explode("/", $url); 
                foreach ($urlData as $urlKey => $urlPart) {
                    if ($urlPart == "start".$this->getGridId()) {
                       $urlData[$urlKey+1] = 0;
                    }
                }
                $urlString = implode("/", $urlData);
                $menuPerPage = $translateObject->translate('show') . ' '
                             . $this->getView()->formSelect(
                                 ...
                                 array('onChange' => "gridAjax('{$this->getInfo("ajax")}',"
                                                     ."'$urlString/perPage" . $this->getGridId() . "/'+this.value);"),
        ...
        if ($actual != 1) {
            if ($this->getInfo("ajax") !== false) {
                $pag = "<a href=\"javascript:gridAjax('{$this->getInfo("ajax")}','$url/start{$this->getGridId()}/"
                     . (($actual - 2) * $ppagina) . "')\">" . $translateObject->translate('previous') . "</a>&nbsp;&nbsp;|&nbsp;" . $pag;
            } else {
                $pag = "<a href=\"$url/start{$this->getGridId()}/" . (($actual - 2) * $ppagina)
                     . "\">" . $translateObject->translate('previous') . "</a>&nbsp;&nbsp;" . $pag;
            }
        }
        ...
        if ($actual != $npaginas) {
            if ($this->getInfo("ajax") !== false) {
                $pag .= "&nbsp;&nbsp;<a href=\"javascript:gridAjax('{$this->getInfo("ajax")}',"
                      . "'$url/start{$this->getGridId()}/" . ($actual * $ppagina) . "')\">" . $translateObject->translate('next')
                      . "</a>";
            } else {

                $pag .= "&nbsp;&nbsp;<a href=\"$url/start{$this->getGridId()}/" . ($actual * $ppagina) . "\">"
                      . $translateObject->translate('next') . "</a>";
            }
        }
            ...
            if ((int) $this->getInfo("limit") > 0) {
                ...
                $result2 = str_replace(
                    array('{{pagination}}',
                          ...
                          '{{perPage}}', '{{pageSelect}}'),
                    array($pag,
                          $translateObject->translate('labelRecords'). ' ' .$registoActual . ' - ' 
    ...
    xmlhttp.onreadystatechange=function () {
        ...
        if (xmlhttp.readyState==4) {
            ...
            if(grid_id=='grid2') {
             ...
             $('#slideBox2-2 tr td a').each(function(index) {
				if($(this).attr('id') == 'showclr')
				{
					$(this).closest('tr').attr('bgcolor','#C3D69B');
				}	
			 });
			 
			 $('a.add').attr('data-fancybox-type','iframe');
			 $('.deleteTableRecord').click( function(){
		...
		$(function() {
		 ...
		 if(grid_id=='maintenancelaneGrid') {
		 
		  $('#lane_maintenance table thead').prepend('<tr><td colspan=\"21\"><ul class=\"createlanelist\"><li><span class=\"title\">ORIGIN</span></li><li><span class=\"title\">DESTINATION</span></li></ul></td></tr>');
		  $('#lane_maintenance table').fixheadertable({ width : 1040, minWidth : 1200 });
